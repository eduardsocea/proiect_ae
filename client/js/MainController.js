...
(function() {
    ...
    function MainController($scope, $filter, dbService, $http) {
        ...
        var onProductCreated = function(data) {
            ...
            dbService.getProducts().then(onProductsLoaded, onError);
            productCreated();
        }
        ...
        var onProductsLoaded = function(data){
            $scope.grouppedProducts  = groupProductsByDate(data);
        ...
        $scope.getSubTotalByDatestamp = function(prod, datestamp) {
                            var sum = 0;
                                if (prod.datestamp == datestamp) {
                                    for (var j = 0; j < prod.products.length; ++j) {
                                        sum += prod.products[j].price;
                                    }
                                }
                            return sum;
                        }
        $scope.getSettingValue = function(settings, datestamp){
                 for(var i = 0; i < settings.length; ++i){
                    if(datestamp.includes(settings[i].date)){
